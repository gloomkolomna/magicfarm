        ...
        return (
          ...
          >
            {p.image_harvested_url ? (
              ...
              <img src={mediaUrl(p.image_grown_url)} alt="" style={{ height: 44, maxWidth: '100%', objectFit: 'contain', display: 'block', margin: '0 auto 4px' }} />
            ) : (
  ...
  return (
    <>
      <LocationMap mapUrl={field.map_url} name={field.name} emoji="🗺️" onBack={() => nav('/fields')}>
            ...
            {/* Зоны питомцев: картинка питомца на всю зону (как у шатров); пустая зона — пунктирная рамка. */}
...
function Modal({ title, onClose, children, wide }: { title: string; onClose: () => void; children: React.ReactNode; wide?: boolean }) {
  return (
    <div style={{ position: 'fixed', inset: 0, zIndex: 60, background: 'rgba(0,0,0,0.6)', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: wide ? 8 : 12 }}>
      <div className="fm-card fm-rise" onClick={(e) => e.stopPropagation()} style={{ width: '100%', maxWidth: wide ? '95vw' : 'min(100%, 560px)', maxHeight: wide ? '95vh' : '85vh', overflowY: 'auto' }}>
