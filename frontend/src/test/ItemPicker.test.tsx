...
describe('ItemPicker', () => {
  it('показывает первые 6 карточек без стрелок, если предметов не больше 6', () => {
    const items = many.slice(0, 6);
  ...
  it('показывает стрелки и счётчик, листает страницы по 6', () => {
    render(<ItemPicker items={many} value={null} onChange={() => {}} />);
    expect(screen.getByText('1 / 3')).toBeTruthy();
    expect(screen.queryByText('Предмет 6')).toBeNull();

    fireEvent.click(screen.getByText('▶'));
    expect(screen.getByText('Предмет 6')).toBeTruthy();
